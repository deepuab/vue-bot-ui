...
export default {
  name: 'RobootBuilder',
  created() {
    // Dispatching action to get data from API once the component is created
    // Using mapActions for dispatching actions
    this.getParts();
  },
  ...
  methods: {
    ...
    addToCart() {
      ...
      + robot.rightArm.cost
      + robot.leftArm.cost;
      // Saving data using name space actions (Action inside robots module)
      // Another form of  dispatching actions
      this.$store.dispatch('robots/addRobotToCart', { ...robot, cost }).then(() => {
